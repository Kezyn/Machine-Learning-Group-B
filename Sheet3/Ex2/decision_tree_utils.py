...
def trainModel(samples, target, attributes, max_depth=2 ** 31, _depth=0):
    ...
    if len(set(s[target_id] for s in
               samples)) == 1:  # if there is only one distinct value of target, return single-node tree containing this value
        root.value = samples[0][target_id]
        return root
    if all(a in {None, target} for a in
           attributes) or _depth >= max_depth:  # if no predictors left or we reached maximum recursion depth, return most frequent value of the target attribute
    ...
    for v in A_values:  # loop over values of the found attribute
        ...
        samples_v = [s[:] for s in samples if
                     s[max_ig_idx] == v]  # build subset filtering samples based on the attribute's value
        ...
        if not samples_v:  # if we got empty subset - add to the subtree child node referring to the most frequent target
            ...
            child = trainModel(samples_v, target, [a for a in attributes if a != A], max_depth,
                               _depth + 1)  # build tree for the subset
...
def evalModel(dataset, train_ids, test_ids, target, max_depth=2 ** 31):
    ...
    predicted = [predict(model, s, attributes) for s in
                 samples_test]  # predict labels for the test split using trained model
    accuracy = sum(1.0 if y == gt else 0.0 for y, gt in zip(ground_truth, predicted)) / len(
        samples_test)  # calculate the accuracy
    return accuracy
...
def main():
    ...
    for sample in samples:
        print(sample, '->', predict(tree, sample,
                                    attributes))  # predict target class for each row in dataset using decision tree trained on the whole dataset
    print('-' * 120)
